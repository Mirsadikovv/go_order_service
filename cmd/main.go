...
import (
	...
	"uzum_orderclone/grpc"
	"uzum_orderclone/grpc/client"
	"uzum_orderclone/storage/postgres"
...
func main() {
	...
	lis, err := net.Listen("tcp", fmt.Sprintf(":%d", cfg.OrderServicePort))
	if err != nil {
		log.Panic("net.Listen", logger.Error(err))
	}

	log.Info("GRPC: Server being started...", logger.String("port", fmt.Sprintf(":%d", cfg.OrderServicePort)))
